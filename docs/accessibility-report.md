...
- `article` — 독립 콘텐츠·카드(메뉴 카드, 이벤트 카드 등)
- `footer` — 연락처·저작권 등 하단
- `nav` — 메뉴·네비게이션
...
- 메뉴 썸네일: `alt="오브 시그니처 사워도우 상품 이미지"`
- Hero 배경(분위기만): `alt=""`
- 로고: `alt="AUBE 오브 로고"` 또는 링크 맥락에 맞는 설명
...
- 토글 버튼: `aria-expanded="true"` / `"false"`, `aria-label`로 열림/닫힘 상태 전달
- 펼쳐지는 영역은 `aria-controls`로 id 연결

**필터(메뉴·이벤트)**

- 필터 버튼: `aria-pressed` 또는 `aria-selected`로 선택 상태 표현
- product.html: 전체/빵/케이크/커피, event.html: 진행중/예정/과거

**폼**

- `label`과 `input`을 id로 연결. 에러 시 `aria-describedby`, `aria-invalid`

## 4. 흐름 — 키보드·포커스

- 링크·버튼·필터·폼 — 키보드만으로 도달·조작 가능
